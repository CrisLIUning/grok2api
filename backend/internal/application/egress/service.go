...
func (s *Service) publicNode(value domain.Node) domain.PublicNode {
	...
	accountBound := false
	if value.EncryptedProxyURL != "" {
		if proxy, err := s.cipher.Decrypt(value.EncryptedProxyURL); err == nil {
			accountBound = strings.Contains(proxy, ProxyAccountPlaceholder)
		}
	}
	return domain.PublicNode{
		...
		AccountBoundProxy: accountBound,
