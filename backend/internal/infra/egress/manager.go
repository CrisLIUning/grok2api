...
// egressStateRepository is optional; relational repo implements clearance helpers.
...
func NewManager(repository repository.EgressRepository, cipher *security.Cipher) *Manager {
	return &Manager{
		repository: repository, cipher: cipher, clients: make(map[clientCacheKey]cachedClient), inflight: make(map[uint64]int),
...
// SetClearanceLock enables cross-instance coordination for fixed egress node clearance.
...
func (m *Manager) UpdateClearanceConfig(value ClearanceConfig) {
	...
	if value.TargetURL == "" {
		value.TargetURL = "https://grok.com"
	}
	previous := m.clearanceConfig
	m.clearanceConfig = value
	if previous.Mode != value.Mode || previous.FlareSolverrURL != value.FlareSolverrURL || previous.TargetURL != value.TargetURL {
		m.clearanceVersion++
	}
}
...
func (m *Manager) acquire(ctx context.Context, scope domain.Scope, affinity string, allowDirect bool, credentialCookies string) (*Lease, bool, error) {
	...
	// Managed mode always enters FlareSolverr lifecycle so stale imported cookies cannot bypass refresh.
	if managedClearance {
		// Prefer not using credential cookies as untrusted bypass when managed.
...
func clearanceCacheKey(nodeID uint64, proxyURL string, sticky bool) string {
	...
	// sticky: bind cache key to rendered proxy (account identity)
	return base + ":" + fmt.Sprintf("%x", sha256.Sum256([]byte(proxyURL)))[:16]
}
