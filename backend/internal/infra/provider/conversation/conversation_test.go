...
func TestConvertResponsesJSONToChatAndMessages(t *testing.T) {
	...
	// FORK DELTA:上游此处断言 cost_in_usd_ticks / num_sources_used /
	// context_details 必须外露。我们是转售面,这些字段被刻意剥掉了
	// (见 usage_privacy_delta_test.go),故改为断言 token 计数正确 + 成本不外露。
	chatUsage := chat["usage"].(map[string]any)
	if chatUsage["prompt_tokens"] != float64(10) || chatUsage["completion_tokens"] != float64(5) {
		t.Fatalf("chat usage = %#v", chatUsage)
	}
	if _, leaked := chatUsage["cost_in_usd_ticks"]; leaked {
		t.Fatalf("成本字段泄漏到转售面: %#v", chatUsage)
	}
	...
	// FORK DELTA:同上,转售面不外露成本字段(见 usage_privacy_delta_test.go)。
	messagesUsage := messages["usage"].(map[string]any)
	if messagesUsage["input_tokens"] != float64(10) || messagesUsage["output_tokens"] != float64(5) {
		t.Fatalf("messages usage = %#v", messagesUsage)
	}
	if _, leaked := messagesUsage["cost_in_usd_ticks"]; leaked {
		t.Fatalf("成本字段泄漏到转售面: %#v", messagesUsage)
	}
}
...
func TestConvertResponsesStreamMessagesInputTokens(t *testing.T) {
	...
	// FORK DELTA:上游此处断言 message_delta 必须保留上游 usage 扩展
	// (cost_in_usd_ticks 等)。我们是转售面,刻意不保留 —— 见
	// usage_privacy_delta_test.go。改为断言它确实没漏出去。
	if strings.Contains(text, `"cost_in_usd_ticks"`) {
		t.Fatalf("成本字段泄漏到流式 message_delta:\n%s", text)
	}
}
