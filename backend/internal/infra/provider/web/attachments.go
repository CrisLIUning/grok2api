...
func validateRemoteAttachmentURLWithResolver(ctx context.Context, raw string, resolver remoteImageResolver, invalid error) (*remoteImageTarget, error) {
	...
	// FORK DELTA:允许任意端口的 HTTPS。自托管重服常用 8443 等非 443 端口,而参考图/
	// 图生视频的素材 URL 正是本网关自己发出去的(publicApiBaseURL)——写死 443 会导致
	// 我们自己拒收自己的素材。SSRF 由下方的公网 IP 校验兜底,端口本身不放大攻击面。
	// 仍强制 https、无 userinfo、主机非空。见 attachments_delta_test.go。
...
func newRemoteImageTarget(original *url.URL, serverName string, address netip.Addr) *remoteImageTarget {
	fetchURL := *original
	// FORK DELTA:沿用原 URL 的端口(缺省才是 443),配合上面放开的非标端口校验。
	port := original.Port()
	if port == "" {
		port = "443"
	}
